...
function ExploreTopics({setCategoryId}) {
  ...
  const imgSrc = title =>
    `/pictures/${encodeURIComponent(title.toLocaleLowerCase())}.png`

  const promptFor = topic =>
    `Give me a clear, beginner-friendly lesson on ${topic}`

  const startTopic = async title => {
    if (busy) return
    try {
      setBusy(true)
      ...
      navigate(
        `/chat?chatId=${encodeURIComponent(r.chatId)}&q=${encodeURIComponent(
          q
        )}`,
        {
  ...
  const Card = ({ title, extra, id }) => (
    <button
      type="button"
      onClick={() => setCategoryId(id)/* startTopic(title) */}
      disabled={busy}
    ...
    >
      ...
      <div className="absolute right-0 bottom-0 pr-4 pb-4 text-stone-200 text-xl sm:text-2xl">
        {title}
      </div>
    </button>
  )
    ...
    <div className="mt-auto pb-4 pt-4 relative">
      ...
      <div className="w-full max-w-4xl mx-auto overflow-hidden">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
          ...
          <Card
            title="Science"
            extra="col-span-1 sm:col-span-2 lg:col-span-1"
          />
        </div>
            ...
            <div key={i} className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
              {row.map((item, j) => (
                ...
                id={item.id}
                  ...
                  title={item.name}
                  extra={
                    j === 2 ? "col-span-1 sm:col-span-2 lg:col-span-1" : ""
                  }
                />
              ))}
            </div>
